...
class Appli(Tk):
	def __init__(self):
		Tk.__init__(self)
		self.title('Programme principal')
		...
		self.label_android_connected = Label(self,text="Android non connecte",fg="red");
		self.label_android_connected.pack(fill=BOTH, expand=True)
		self.label_log = Label(self,text="",fg="red");
		self.label_log.pack(fill=BOTH, expand=True)


		self.frame2 = Frame(self, relief=RAISED, borderwidth=1)
		self.frame2.pack(fill=BOTH, expand=True)
		self.label_info_spin = Label(self.frame2,text="Duree de l'enregistrement (en s)");
		self.label_info_spin.pack(fill=BOTH, expand=True)
		self.spin_box_duree = Spinbox(self.frame2, from_=1, to=10)
		self.spin_box_duree.pack(fill=BOTH, expand=True)
		...
		self.frame3 = Frame(self, relief=RAISED, borderwidth=1)
		self.frame3.pack(fill=BOTH, expand=True)
		self.button_start_kinect = Button(self.frame3,text="Demarrer Kinect",command=self.startKinect)
		self.button_start_kinect.pack(fill=BOTH, expand=True)
		self.button_start_recording = Button(self.frame3,text="Demarrer enregistrement",command=self.startRecording)
		self.button_start_recording.pack(fill=BOTH, expand=True)
		self.button_start_recording.config(state="disabled")
		self.button_traitement = Button(self.frame3,text="Traitement",command=self.startTraitement)
		self.button_traitement.pack(fill=BOTH, expand=True)
		...
		self.label_dist_left = Label(self.frame,text="");
		self.label_dist_left.pack(fill=BOTH, expand=True)
		self.label_dist_right = Label(self.frame,text="");
		self.label_dist_right.pack(fill=BOTH, expand=True)
		self.label_result = Label(self.frame,text="");
	...
	def androidConnection(self):
		...
		print "Attente de connexion sur RFCOMM %d" % port
		...
		if self.isOpen:
			...
			self.label_android_connected.config(text="Android "+self.client_info[0]+" connecte",fg="green");
			self.isAndroidConnected = True;
	...
	def startKinect(self):
		...
		time.sleep(2)
		self.shm = mmap.mmap(0, 512, "Local\\Test") 
		if self.shm:
			time.sleep(0.5)
	...
	def startRecording(self):
		...
		if not self.isKinectTracking():
			...
			self.nb_sec_rec = int(self.spin_box_duree.get())
			print "nb_sec_rec = ",self.nb_sec_rec
	...
	def startAndroidRecording(self):
		...
		print self.nb_sec_rec
		self.client_sock.send(chr(self.nb_sec_rec))	
	...
	def threadStartTraitement(self):"""
		...
		# interpolation et re-echantillonnage a 50Hz
		...
		if dist_left<dist_right:
			...
			index = dist_left_idx
			left_str = "- distance main gauche : " + "{:6.2f}".format(dist_left)
			right_str = "- distance main droite : " + "{:6.2f}".format(dist_right)
			res_str = "==> main GAUCHE"
			result = "gauche"
		...
		self.label_dist_left.config(text=left_str)
		self.label_dist_right.config(text=right_str)
		self.label_result.config(text=res_str,width=15)

		if result=="gauche":
			self.label_dist_left.config(fg="black",bg="green")
			self.label_dist_right.config(fg="black",bg="red")
		else:
			self.label_dist_right.config(fg="black",bg="green")
			self.label_dist_left.config(fg="black",bg="red")


		self.label_dist_left.update()
		self.label_dist_right.update()
		self.label_result.update()

		
		
		fig = plt.figure() 
		fig.canvas.set_window_title('Mise en correspondance') 
		plt.subplot(231)
		plt.xlabel('temps (s)')
		plt.ylabel('position (pixel)')
		plt.title('Mouvement main gauche')
